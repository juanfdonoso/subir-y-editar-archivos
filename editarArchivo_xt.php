...
<body>
    ...
    if (isset($_POST["id"])){
        ...
        if ($error != 0){
            ...
            echo 'window.location.assign("editarArchivo.php?error='.$error.'&id='.$idParaBorrar.'");';
            echo '</script>';
        }else{
            ...
            if (move_uploaded_file($tempName, $archivoParaSubir)){
                ...
                //borramos el archivo que se quería editar (el que se va a reemplazar)
                //identificamos qué archivo es, a través de su id
                $sql2 = "select archivo, tipo from juanf_A_archivos where idArchivo = ".$idParaBorrar;
                $rs2 = ejecutar($sql2);
                $dato2 = mysqli_fetch_array($rs2);

                //dependiendo del tipo de archivo, definimos la ruta para borrarlo
                if ($dato2["tipo"] == "foto"){
                    $ruta = $rutaFoto;

                }else if ($dato2["tipo"] == "pdf"){
                    $ruta = $rutaPDF;

                }else if ($dato2["tipo"] == "video"){
                    $ruta = $rutaVideo;
                }

                $archivoParaBorrar = $dato2["archivo"];

                //borramos el archivo
                if (unlink($ruta.$archivoParaBorrar)){
                    // borrar el registro del archivo de la BD
                    $sql = "delete from juanf_A_archivos where idArchivo = ".$idParaBorrar;
                    $nada = ejecutar($sql);

                }else{
                    //generamos una alerta de error
                    echo '<script language="javascript">';
                    echo 'alert("El archivo que se deseaba editar no pudo borrarse del servidor. Contacte al administrador del sistema")';
                    echo '</script>';
